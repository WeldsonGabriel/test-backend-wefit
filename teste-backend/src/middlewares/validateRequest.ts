...
const validateCreateCadastro = (req: Request, res: Response, next: NextFunction) => {
  if (req.method === 'GET' || req.method === 'DELETE' || (req.method === 'PATCH' && req.path.includes('/persons/reactivate'))) {
    return next(); // Skip validation for GET, DELETE, and specific PATCH requests
  }
